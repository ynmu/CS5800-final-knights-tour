...
def update():
    ...
    for pos, (x, y) in enumerate(moves[:step]):
...
moves = [[0, 0] for _ in range(ROWS * COLS)]
if solution_board is not None:
    for i in range(ROWS):
        for j in range(COLS):
            moves[solution_board[i][j]] = [i, j]
else:
    sys.exit(f'No solution found for {ROWS}x{COLS} board, '
             f'starting from row {START_ROW}, column {START_COL}')
...
breverse.on_clicked(reset_step)
